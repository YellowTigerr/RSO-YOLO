...
from utils.general import (CONFIG_DIR, FONT, LOGGER, check_font, check_requirements, clip_boxes, increment_path,
                           is_ascii, xywh2xyxy, xyxy2xywh,xyxyxyxy2xywhr,xywhr2xyxyxyxy)
...
class Annotator:
    ...
    def poly_label(self, poly, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if label:
            ...
            cv2.putText(self.im, label, (x_label, y_label + h), 0, self.lw / 3, txt_color, thickness=tf,
                        lineType=cv2.LINE_AA)
        # self.im = self.im if isinstance(self.im, Image.Image) else Image.fromarray(self.im)
...
def scale_polys(img1_shape, polys, img0_shape, ratio_pad=None):
    ...
    xywh[..., 0] -= pad[0]  # x padding
    xywh[..., 1] -= pad[1]  # y padding
    xywh[..., :4] /= gain
    ...
    # clip_boxes(xywh, img0_shape)
    boxes = torch.cat((xywh,angle),dim=1)
    polys = xywhr2xyxyxyxy(boxes).view(-1,8)
    return polys
