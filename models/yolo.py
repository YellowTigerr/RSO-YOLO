...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        self.cv2 = nn.ModuleList(       # Bbox
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)
        self.cv3 = nn.ModuleList(       # cls
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, DualDetect, TripleDetect, DDetect, DualDDetect, TripleDDetect, Segment, ObbDetect)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.inplace = self.yaml.get('inplace', True)  # 原地操作

        # Build strides, anchors
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, DDetect, Segment, ObbDetect)):
            s = 256  # 2x min stride
...
class ObbModel(DetectionModel):
    def __init__(self, cfg='yolo-obb.yaml', ch=3, nc=None, anchors=None):
        super().__init__(cfg, ch, nc, anchors)
...
class ObbDetect(nn.Module):
    # YOLO Obb head for obb models
    dynamic = False  # force grid reconstruction
    export = False  # export mode
    shape = None
    anchors = torch.empty(0)  # init
    ...
    def __init__(self, nc=80, ne=1, ch=(), inplace=True):
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(ch)  # number of detection layers
        self.reg_max = 16
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            self.shape = shape
        # dfl_box
        box, angle, cls = torch.cat([xi.view(shape[0], self.no, -1) for xi in x], 2).split(
            (self.reg_max * 4, self.ne, self.nc), 1)
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5) (5: x y w h 置信度)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        # TODO: channel, gw, gd
        elif m in {Detect, DualDetect, TripleDetect, DDetect, DualDDetect, TripleDDetect, Segment, ObbDetect}:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default= r'E:\study\yolov9-rotated\yolov9-rotated\models\obb\yolov9obb.yaml', help='model.yaml')
