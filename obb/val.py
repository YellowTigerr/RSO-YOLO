...
from utils.general import (LOGGER, TQDM_BAR_FORMAT, Profile, check_dataset, check_img_size, check_requirements,
                           ...
                           print_args,  xywh2xyxy, xyxy2xywh)
from utils.metrics import box_iou, ConfusionMatrix, ap_per_class
from utils.obb.utils import rotated_non_max_suppression
from utils.obb.plots import poly2hbb,scale_polys,rbox2poly,scale_coords,plot_images,output_to_target
...
def run(
    ...
    s = ('%22s' + '%11s' * 6) % ('Class', 'Images', 'Instances', 'P', 'R', 'HBBmAP50', 'HBBmAP50-95')
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[2]:
            preds = rotated_non_max_suppression(preds,
                                        conf_thres,
                                        iou_thres,
                                        labels=lb,
                                        multi_label=True,
        ...
        for si, pred in enumerate(preds):
            ...
            if single_cls:
                pred[:, 6] = 0
            predn = pred.clone()
            poly = xywhr2xyxyxyxy(pred[:, :5]).view(-1, 8)  # (n, 8)
            ...
            stats.append((correct.cpu(), pred_poly[:, 8].cpu(), pred_poly[:, 9].cpu(), labels[:, 0]))
